 ...
 //var model = new THREE.Mesh( geometry, new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture( 'texture.jpg' ) } ) );
		...
		character1 = new Physijs.BoxMesh(new THREE.BoxGeometry(20,40,20), character_material, 10);
...
//scene.add(testbox);
domEvents.addEventListener(testbox, 'click', function(event){
				controls.enabled = false;
				trans_control.attach(testbox);
				trans_control.enabled = true;
							console.log((testbox).position);
							console.log((testbox).rotation);
							console.log((testbox).scale);
          ...
          //panoMesh.rotation.y = time;
			...
			controller1annie2 = new TWEEN.Tween( controller1.position ).to( {
				...
				*/	
				...
				function camtween2(){
					camera.lookAt(shape_mesh4.position);
					render();
				};
